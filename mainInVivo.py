...
modelTypes = ["compReal", "Ma_4Convs", "Tapper"]  # currently missing ["Ma_2Convs", "realReal", "Tapper", "compComp", "Ma_4Convs", "compReal"]
netTypes = ["freq", "phase"]  # ["freq", "phase"]
offsetSize = ["Small", "Medium", "Large"]

batch_size, learn_r = 64, 0.0001
nmb_epochs = [200]
loss_fn = nn.L1Loss()
...
phaseLabels_dev = np.load(f"{dataDir}Simulated/Corrupt/Sim2_5_{dataType}/TruePhaseLabels_Sim2_5_{dataType}_Dev.npy")[:, 0, :]          #(#spec points, #trans)
...
for epochNUMS in nmb_epochs:
    for model_name in modelTypes:                                           # models: MLP, CNN, CR-CNN
        for net in netTypes:                                                # nets: frequency and phase
            run_name = f'{net}_InVivo_{model_name}_lr25PercentMaxOnly_B64Adam_E200'
            print(run_name)
            predTrainLabels, predValLabels = 0, 0
...
#########################################################################################################################
    # Test based on offset size (small, medium, large)
            for size in offsetSize:
                ...
                # Spectra Normalization
                # Percentile
                on_all_specs_test = (on_all_specs_test) / ((np.percentile(np.abs(on_all_specs_test), 95, axis=2, keepdims=True)))
                off_all_specs_test = (off_all_specs_test) / ((np.percentile(np.abs(off_all_specs_test), 95, axis=2, keepdims=True)))
