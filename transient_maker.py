...
class TransientMaker:
    ...
    def __init__(self, groundTruthNum, ON_loc, OFF_loc, ppm_loc, time_loc, transient_count):
        # load data
        base_on_fids = np.loadtxt(ON_loc, dtype=complex, delimiter=",")
        base_off_fids = np.loadtxt(OFF_loc, dtype=complex, delimiter=",")
    ...
    def add_freq_shift_random(self, freq_var=5):
        ...
        noise = stats.truncnorm((-freq_var - 0) / freq_var, (freq_var - 0) / freq_var, loc=0, scale=freq_var)
        noise = noise.rvs(size=(self.fids.shape[0], 1, self.transient_count))
        # noise = noise.rvs(size=(self.fids.shape[0], self.fids.shape[1], self.transient_count))
    ...
    def add_phase_shift_random(self, phase_var=20):
        ...
        noise = stats.truncnorm((-phase_var - 0) / phase_var, (phase_var - 0) / phase_var, loc=0, scale=phase_var)
        noise = noise.rvs(size=(self.fids.shape[0], 1, self.transient_count))
    ...
    def get_SNR(self, specs):
        ...
        for i in range(0, specs.shape[2]):
            mean_specs = np.real(specs[0, :, i])
            ...
            allSNR.append((all_MAXy) / (2 * allSNR_stdev))
